...
(function () {
  ...
  function fail(msg) {
    root.innerHTML = '<div class="max-w-site mx-auto px-7 py-20 text-center"><p class="text-[15px] text-neutral-600">' + esc(msg) + '</p>'
      + '<a href="/news" class="btn border border-black/15 bg-white hover:border-shad hover:text-shad mt-6"><i class="ti ti-list"></i>NEWS一覧へ</a></div>';
  }
  if (!id) { fail("記事が指定されていません。"); return; }

  /* 品番 → 型番（EC商品リンクを自サイトの商品ページに向ける） */
  function codeMap() {
    return Promise.all(["/data/catalog/cards.json", "/data/catalog/products.json"].map(function (u) { return fetch(u).then(function (r) { return r.ok ? r.json() : {}; }).catch(function () { return {}; }); }))
      .then(function (rs) {
        var m = {};
  ...
  function fixHref(href, codes) {
    ...
    if (m) { var code = codes[m[1].toUpperCase()]; if (code) return { href: "/product/" + code.toLowerCase(), ext: false }; }
