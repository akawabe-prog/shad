# SHAD JAPAN 本番アップロード手順（shad.customjapan.net）
...
`dist/shad/` の中身を、**shad.customjapan.net のドキュメントルート直下**にそのまま配置してください。
...
- 差出人: `From: noreply@shad.customjapan.net`（Reply-To は入力メールアドレス）
...
- 送信ドメイン `shad.customjapan.net` からの `noreply@` 差出人が SPF 等で許可されていること（eXs の `noreply@exs.customjapan.net` と同様の設定）。
