...
var CARDS = {};
Promise.all([
  fetch('/shad/data/fitment/reverse_data.json').then(r => r.json()),
  fetch('/shad/data/catalog/cards.json').then(r => r.ok ? r.json() : {}).catch(() => ({}))
])
...
(function () {
  ...
  function showSelectedBike(rows, kitLabel) {
    ...
    box.innerHTML = ''
      ...
      +   '<a class="fit-from-back" href="/shad/fitment?bike=' + encodeURIComponent(FROM_BIKE) + '">'
  ...
  Promise.all([get("/shad/data/fitment/product_index.json"),
               get("/shad/data/fitment/reverse_data.json")]).then(function (res) {
