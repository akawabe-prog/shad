...
(function () {
  ...
  function imgUrl(p) {
    if (!p) return "";
    return (/^https?:/.test(p) || p.indexOf("/shad/img/") === 0) ? p : IMG_HOST + p;   // /img/… はサイト内画像
  }
  ...
  function applyGallery(variant) {
    ...
    //   window.SHAD_GALLERY = { "<品番>": ["/shad/img/…", …], … }   （例：TR46 のカラー別パックショット）
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    Promise.all([
      loadJSON("/shad/data/catalog/products.json"),
      loadJSON("/shad/data/catalog/accessory_index.json"),
      loadJSON("/shad/data/catalog/accessories.json"),
      loadJSON("/shad/data/catalog/others.json"),
    ]).then(function (res) {
      var products = res[0] || {};
      var index = res[1] || {};
