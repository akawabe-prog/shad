...
(function () {
  ...
  var local = {};
  document.querySelectorAll('a[href^="/shad/news/cj-"]').forEach(function (a) { var m = a.getAttribute("href").match(/cj-(\d+)/); if (m) local[m[1]] = true; });

  function card(a) {
    var isLocal = !!local[String(a.id)];
    /* まだ静的ページを生成していない新着は、APIから本文を取るサイト内のライブ記事ページへ */
    var href = isLocal ? "/shad/news/cj-" + a.id : "/shad/news/article?id=" + a.id;
    var attrs = "";
    var thumb = a.image
  ...
  if (rail) {
    load(false).then(function (items) {
      ...
      if (tags && tags.length) {
        ...
        var view = host.querySelector('a[href="/shad/news"]');
        cs.forEach(function (c) { var a = document.createElement("a"); a.href = "/shad/news?cat=" + encodeURIComponent(c); a.className = "tag hover:border-shad hover:text-shad transition"; a.textContent = c; host.insertBefore(a, view); });
      }
    }).catch(function () {});
  }
